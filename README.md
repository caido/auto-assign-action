...
Use a release (or commit) whose bundled `dist/index.js` includes this feature. GitHub Actions executes that bundle, not the TypeScript source. A source-only commit with an older bundle can report `candidates.filter is not a function` for the object syntax below. When upgrading, update your workflow's pinned action version or SHA to the release-built commit.
...
GitHub REST requests use `openapi-fetch` with types generated by `openapi-typescript`. The generated declarations in `src/generated/github.d.ts` are checked in, so normal builds, tests, packaging, and action runs do not download an API specification.

To regenerate them explicitly, run:

```bash
npm run generate:github-types
```

The command uses GitHub's official bundled OpenAPI 3.0 description pinned to commit [`3cef12e8a02d612ad032473d4fb87266f2befeae`](https://github.com/github/rest-api-description/tree/3cef12e8a02d612ad032473d4fb87266f2befeae) and `openapi-typescript` 7.13.0. Update both the immutable URL in `package.json` and the checked-in declaration when intentionally moving the specification pin.

### Network proxy support

`openapi-fetch` uses the native `fetch` implementation included with Node.js 24; the action does not bundle the npm `undici` package. Unlike `@actions/github`, native fetch does not automatically use `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` unless Node's environment proxy support is enabled when the action process starts. Self-hosted runners that require a proxy can set `NODE_USE_ENV_PROXY=1` in the runner environment (or start Node with `--use-env-proxy`).
